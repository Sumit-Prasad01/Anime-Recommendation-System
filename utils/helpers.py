...
def find_similar_user(item_input, path_user_weights, path_user2user_encoded, path_user2user_decoded, n = 10,  return_dist = False, neg = False):
    try:
...
def get_user_recommendations(similar_users, user_pref, path_df, path_synopsis_df, path_rating_df, n = 10):
    ...
    if anime_list:
        ...
        for i, anime_name in enumerate(sorted_list.index):
            n_usre_pref = sorted_list[sorted_list.index == anime_name].values[0][0]
    ...
    return pd.DataFrame(recomended_animes).head(n)